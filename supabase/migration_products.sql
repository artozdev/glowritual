...
-- Si la table existait déjà (ancien schéma), on ajoute les colonnes manquantes
-- (idempotent). `is_routine` notamment n'existait pas → cause de l'erreur au seed.
alter table public.products add column if not exists also_helps      text[] not null default '{}';
alter table public.products add column if not exists key_ingredients jsonb  not null default '[]';
alter table public.products add column if not exists skin_types      text[];
alter table public.products add column if not exists gender          text default 'all';
alter table public.products add column if not exists vegan           boolean default false;
alter table public.products add column if not exists affiliate_url   text;
alter table public.products add column if not exists is_routine      boolean not null default false;
...
values
  ...
  ('sephora-gua-sha','Gua Sha Quartz Rose','Sephora','tool','contour','{firmness}',15,4.5,650,'{}','https://www.sephora.fr/p/gua-sha-quartz-rose---visage-et-cou-532082.html','{}','[{"name":"Massage gua sha","benefit":"tonifie, lisse, draine le contour"}]','Sur huile/sérum, 5 min, mouvements ascendants.','{}','all',false,false),
  ('jawliner-3','Jawliner 3.0','Jawliner','tool','contour','{}',30,4.3,1200,'{}','https://jawliner.com/fr/products/jawliner-3-0','{}','[{"name":"Entraînement mâchoire","benefit":"travaille la ligne de mâchoire"}]','Quelques minutes par jour.','{}','all',false,false),
  ('az-serum-cils-ricin','Sérum cils fortifiant au Ricin BIO','Aroma-Zone','serum','hair','{}',8,4.4,900,'{Bio}','https://www.aroma-zone.com/info/fiche-technique/serum-cils-au-ricin','{"huile de ricin",tocopherol}','[{"name":"Huile de ricin","benefit":"fortifie, volume et longueur des cils"}]','Le soir, au pinceau.','{}','all',true,false),
  ...
  ('az-serum-keratine-peptides','Sérum Cheveux Kératine AA & Peptides','Aroma-Zone','serum','hair','{}',16,4.4,260,'{}','https://www.aroma-zone.com/info/fiche-technique/serum-cheveux-keratine-aa-peptides','{keratine,peptides,aqua}','[{"name":"Kératine + Peptides","benefit":"répare la fibre capillaire"}]','Sur longueurs et pointes.','{}','all',false,false),
  ('az-huile-barbe','Huile à barbe Acide hyaluronique & Chanvre BIO','Aroma-Zone','oil','beard','{}',12,4.4,230,'{Bio}','https://www.aroma-zone.com/info/fiche-technique/huile-a-barbe-acide-hyaluronique-chanvre-bio','{"huile de chanvre","acide hyaluronique",tocopherol}','[{"name":"Chanvre + AH","benefit":"nourrit la barbe, hydrate la peau"}]','Quelques gouttes sur barbe propre.','{}','male',true,false),
  ('nutripure-magnesium','Magnésium³ Poudre','Nutripure','supplement','wellness','{}',19,4.7,1600,'{}','https://www.nutripure.fr/fr/sante/251-magnesium-poudre.html','{magnesium,taurine,"vitamine b6"}','[{"name":"Magnésium (3 formes)","benefit":"réduit fatigue et stress"}]','1 dose/jour en cure.','{}','all',true,false),
  ('dailylab-lacher-prise','Complexe Lâcher Prise','Daily Lab','supplement','wellness','{}',24,4.5,410,'{Vegan}','https://dailylab.com/products/complexe-lacher-prise','{"plantes adaptogenes",magnesium}','[{"name":"Plantes adaptogènes","benefit":"aide à gérer le stress"}]','En cure, selon posologie.','{}','all',true,false)
